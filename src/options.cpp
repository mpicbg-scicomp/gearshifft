...
int Options::parse(std::vector<char*>& _argv, std::vector<char*>& _boost_vargv) {
  ...
  desc.add_options()
    ...
    ("device,d", po::value<std::string>(&device_)->default_value("gpu"), "Compute device = (gpu|cpu|acc|<ID>). If device is not supported by FFT lib, then it is ignored and default is used. Use 4xcpu in order to use 4 processors (if your platform has that many)")
